...
**버전**: 1.1  
**작성일**: 2026-05-26  
**기반 문서**: 사용자 제공 SDD 초안, `소프트웨어공학_프로젝트.docx (SRS)`
...
- JSON 로그 파일의 최상위 구조는 배열 형식으로 통일한다.
- 결과 파일은 사용자가 리포트 탭에서 수동으로 저장한다.
- 평가 진행률은 파일 로드·검증·매칭·완료의 단계 기반으로 표시한다.
...
- 기본 선택 상태는 미선택이며, 필수 파일 4종이 모두 지정된 경우에만 평가 실행이 가능하다.
...
| 리포트 자동 저장 여부 | 확정 | 수동 저장을 기본으로 하고 자동 저장은 비활성으로 둔다. | 리포트 재생성 빈도와 저장 위치 정책이 확정될 때 | ReportExporter, UI 버튼, 상태 메시지 | SRS, SDD, STD | 저장 동작 테스트, 자동 저장 UI 테스트 |
| 진행률 계산 기준 | 확정 | 처리 단계 수 기준의 단순 진행률을 임시 사용한다. | 전체 처리 단계와 사용자 피드백 정책이 확정될 때 | View 진행바, AppController 상태 전이 | SRS, SDD, STD | 진행률 표시 테스트 |
| UI 세부 배치 | 확정 | 현재 탭 구조를 유지하고 세부 배치는 추후 조정한다. | 화면 구성 확정 또는 사용성 피드백 반영 시 | View 레이아웃, 탭 순서, 위젯 배치 | SDD, STD | UI 배치 확인 테스트 |
| 이미지 미리보기 확대 기능 | 확정 | 기본 미리보기만 제공하고 확대는 미지원으로 둔다. | 검토자가 원본 확인을 자주 요구할 때 | View, 이미지 표시 방식 | SDD, STD | 이미지 미리보기 테스트 |
| 시각화 그래프 종류 추가 여부 | 확정 | 현재 그래프만 제공하고 추가 그래프는 후보로 둔다. | 성능, 가독성, 분석 요구가 확정될 때 | View, ReportExporter, 데이터 집계 | SRS, SDD, STD | 그래프 표시 테스트 |
| 대용량 입력 최적화 방식 | 확정 | 순차 처리와 기본 캐시 수준으로 유지한다. | 데이터 규모가 현재 기준을 초과하는 운영 조건이 확정될 때 | FileLoader, Evaluator, 성능 | SDD, STD | 대용량 성능 테스트 |
| collision_log.csv를 필수 입력으로 계속 유지할지 | 확정 | 현재는 필수 입력으로 유지한다. | 충돌 로그가 선택 입력으로 변경될 가능성이 검토될 때 | 입력 검증, 누락 판정, 점수 계산 | SRS, SDD, STD | collision_log 필수 여부 테스트 |
| CSV 일부 행 오류를 어디까지 Recoverable로 처리할지 | 확정 | 헤더와 필수 필드 오류는 실패, 개별 데이터 행 오류는 경고 후 스킵으로 둔다. | 오류 복구 허용 범위가 운영 기준으로 확정될 때 | FileLoader, Validator, 오류 메시지 | SDD, STD | 행 오류 복구 테스트 |
| 저장 실패 후 상태를 Error로 둘지 Completed로 복귀할지 | 확정 | 평가 완료와 저장 실패를 분리해 저장 실패는 Error로 둔다. | 상태 전이 UX가 확정될 때 | 상태 전이, UI 메시지, 저장 흐름 | SDD, STD | 상태 전이 테스트 |
| 테스트 데이터셋 규모 기준 | 확정 | 소형, 중형, 대형의 최소 대표 크기를 각각 유지한다. | 성능 목표와 회귀 테스트 기준이 확정될 때 | tests 데이터, 성능 검증 | STD, tests 설계 | 데이터셋 규모 테스트 |
| 성능 검증 환경 기준 | 확정 | 기본 개발 환경 기준을 임시 기준으로 둔다. | 운영 환경 또는 기준 장비가 확정될 때 | 성능 수치, 재현성, 비교 기준 | SRS, SDD, STD | 성능 검증 테스트 |
...
|---|---|---|---|---|---|
| mission_config.json 구조가 확정되어 있는가 | 필수 필드와 중첩 구조가 문서에 고정되어 있어야 한다. | 입력 파일 스키마 상세 설계, 용어 및 데이터 사전 | FileLoader, Validator, MissionConfig | 확인 | 구조 변경 가능성이 있으면 구현을 시작하지 않는다. |
| capture_log.csv 구조가 확정되어 있는가 | 필수 컬럼과 타입, 시간 단위가 문서에 고정되어 있어야 한다. | 입력 파일 스키마 상세 설계, 용어 및 데이터 사전 | FileLoader, Validator, CaptureRecord | 확인 | CSV 컬럼명 변경은 입력, 테스트, 리포트에 영향을 준다. |
| collision_log.csv 구조가 확정되어 있는가 | 필수 컬럼과 collision 해석 규칙이 문서에 고정되어 있어야 한다. | 입력 파일 스키마 상세 설계, 용어 및 데이터 사전 | FileLoader, Validator, CollisionRecord | 확인 | 충돌 의미가 바뀌면 감점과 집계가 함께 바뀐다. |
| 단위가 meter, degree, second로 일관되는가 | 위치, 각도, 시간의 단위가 문서 전반에서 동일해야 한다. | 데이터 모델 관계도, 용어 및 데이터 사전, 요구사항-설계 상세 추적성 | Model, Evaluator, ScoreCalculator | 확인 | 단위 불일치는 계산과 리포트 해석을 깨뜨린다. |
| success 판정 조건이 확정되어 있는가 | 위치, 방향, 시간 조건의 결합 규칙이 하나로 정리되어야 한다. | 알고리즘 상세 설계, 설계 결정 기록, 용어 및 데이터 사전 | Evaluator, TargetResult | 확인 | success 정의가 흔들리면 결과 집계가 불안정해진다. |
| missing 처리 규칙이 확정되어 있는가 | 매칭 실패와 기한 초과가 missing에 어떻게 반영되는지 정해져야 한다. | 알고리즘 상세 설계, 오류 처리 매트릭스 | Evaluator, Matcher, TargetResult | 확인 | missing 규칙은 평균 오차와 최종 점수에 영향을 준다. |
| timeout 감점 규칙이 확정되어 있는가 | 시간 초과의 판정과 감점식이 문서에 고정되어야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | ScoreCalculator, TargetResult | 확인 | 시간 기준이 바뀌면 STD와 테스트도 함께 수정된다. |
| collision 감점 규칙이 확정되어 있는가 | collision=true의 반영 범위와 감점식이 고정되어야 한다. | 오류 처리 매트릭스, 설계 결정 기록 | ScoreCalculator, CollisionRecord | 확인 | 충돌 감점은 결과 요약과 상세 결과 모두에 영향을 준다. |
| 리포트 출력 파일 4종이 확정되어 있는가 | eval_result.json, eval_summary.json, eval_result.csv, eval_detail.csv가 고정되어야 한다. | 파일명 및 리포트 용어, 리포트 저장 상세 설계 | ReportExporter | 확인 | 파일명이 바뀌면 외부 문서와 저장 테스트가 함께 바뀐다. |
| Error 상태 처리 방식이 확정되어 있는가 | 저장 실패, 입력 실패, 계산 실패의 상태 전이가 문서에 정리되어야 한다. | 평가 상태 전이도, 오류 처리 매트릭스, 운영 절차 및 사용자 작업 흐름 | AppController, View | 확인 | 상태 전이 기준이 없으면 UI와 테스트가 흔들린다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| SRS, SDD, STD의 필드명이 일치하는가 | 동일 필드가 세 문서에서 같은 철자로 쓰여야 한다. | 요구사항-설계 상세 추적성, 용어 및 데이터 사전 | SRS, SDD, STD | 확인 | 필드명 불일치는 구현 오류보다 먼저 정리해야 한다. |
| deduction 필드명이 통일되어 있는가 | position_deduction, yaw_deduction, pitch_deduction, total_deduction가 일관되어야 한다. | 용어 및 데이터 사전, 알고리즘 상세 설계 | ScoreCalculator, EvalResult | 확인 | 감점 필드는 리포트와 집계의 기준이 된다. |
| target_id 의미가 문서 전반에서 동일한가 | target 식별자와 표시용 이름이 구분되어야 한다. | 용어 및 데이터 사전, 데이터 모델 관계도 | MissionConfig, TargetPoint | 확인 | target_id는 매칭과 리포트의 연결 키이다. |
| collision 처리 기준이 문서 전반에서 동일한가 | collision=true의 해석과 감점 반영이 동일해야 한다. | 용어 및 데이터 사전, 오류 처리 매트릭스 | CollisionRecord, ScoreCalculator | 확인 | 충돌 기준이 흔들리면 집계가 무의미해진다. |
| average error 계산에서 missing target 제외 원칙이 일치하는가 | 평균 오차는 missing target을 제외해야 한다. | 용어 및 데이터 사전, 요구사항-설계 상세 추적성 | EvalResult, ScoreDetail | 확인 | 평균값 계산 기준은 시험과 리포트에 동일해야 한다. |
| UI 탭 이름이 문서 전반에서 동일한가 | 파일 선택, 임무 설정 확인, 평가 실행, 결과 요약, 상세 결과, 시각화, 리포트 저장 이름이 고정되어야 한다. | 운영 절차 및 사용자 작업 흐름, UI 탭별 상세 설계 | View, AppController | 확인 | 탭 이름 변경은 사용자 작업 흐름과 시험 항목에 영향을 준다. |
| 출력 파일명이 문서 전반에서 동일한가 | eval_result.json 등 확정된 이름이 일치해야 한다. | 용어 및 데이터 사전, 파일명 및 리포트 용어 | ReportExporter | 확인 | 출력 파일명은 외부 연동과 저장 검증의 기준이다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| MissionConfig 구조가 확정되었는가 | 목표, 허용 오차, 가중치, 감점 정책의 필드가 고정되어야 한다. | 데이터 모델 관계도, 용어 및 데이터 사전 | MissionConfig | 확인 | 데이터 모델은 입력과 출력의 중심이다. |
| TargetPoint 구조가 확정되었는가 | target_id, position, direction, time_limit이 고정되어야 한다. | 데이터 모델 관계도, 용어 및 데이터 사전 | TargetPoint, Tolerance | 확인 | target 구조는 매칭과 판정의 핵심이다. |
| CaptureRecord 구조가 확정되었는가 | timestamp, position, direction, image_path가 고정되어야 한다. | 데이터 모델 관계도, 용어 및 데이터 사전 | CaptureRecord | 확인 | capture 구조는 입력 로그와 미리보기를 연결한다. |
| CollisionRecord 구조가 확정되었는가 | collision 필드와 관련 메타데이터가 고정되어야 한다. | 데이터 모델 관계도, 용어 및 데이터 사전 | CollisionRecord | 확인 | 충돌 구조는 감점과 집계의 기준이다. |
| EvalResult 구조가 확정되었는가 | total_targets, success_count, missing_count, timeout_count, collision_count, final_score가 고정되어야 한다. | 결과 집계 설계, 용어 및 데이터 사전 | EvalResult | 확인 | 결과 모델은 리포트와 UI의 기반이다. |
| ScoreDetail 구조가 확정되었는가 | position_error, yaw_error, pitch_error, 각 감점 필드가 고정되어야 한다. | 결과 집계 설계, 용어 및 데이터 사전 | ScoreDetail | 확인 | 상세 점수는 설명 가능성과 추적성의 기준이다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| mission_config.json 로드 경로가 확정되었는가 | 입력 스키마와 파일 선택 흐름이 정해져 있어야 한다. | 입력 파일 스키마 상세 설계, 운영 절차 및 사용자 작업 흐름 | FileLoader, View | 확인 | 경로 해석 규칙이 먼저 고정되어야 한다. |
| capture_log.csv 로드 경로가 확정되었는가 | capture 로그의 컬럼 구조와 파싱 방식이 정해져 있어야 한다. | 입력 파일 스키마 상세 설계, 요구사항-설계 상세 추적성 | FileLoader, Validator | 확인 | 로그 파일 형식 변경은 파서에 직접 영향을 준다. |
| collision_log.csv 로드 경로가 확정되었는가 | 충돌 로그의 필수 여부와 컬럼 구조가 정해져 있어야 한다. | 입력 파일 스키마 상세 설계, 운영 절차 및 사용자 작업 흐름 | FileLoader, Validator | 확인 | 충돌 로그는 필수 입력 정책과 연결된다. |
| 인코딩 처리 기준이 확정되었는가 | UTF-8 처리 원칙이 문서에 명시되어 있어야 한다. | 성능 및 확장성 설계, 요구사항-설계 상세 추적성 | FileLoader | 확인 | 인코딩 기준이 없으면 입력 실패가 늘어난다. |
| 이미지 경로 처리 기준이 확정되었는가 | image_path는 평가 계산과 분리되어 있어야 한다. | 용어 및 데이터 사전, 운영 절차 및 사용자 작업 흐름 | FileLoader, View | 확인 | 미리보기와 계산을 혼동하면 안 된다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| Fatal과 Recoverable 구분이 확정되었는가 | 입력 구조 실패와 행 단위 오류의 경계가 정해져 있어야 한다. | 오류 처리 매트릭스, 미확정 사항 및 변경 관리 설계 | Validator, AppController | 확인 | 구분이 불명확하면 상태 전이가 흔들린다. |
| Warning 기준이 확정되었는가 | image_path 문제와 미리보기 실패의 경고 기준이 정해져 있어야 한다. | 오류 처리 매트릭스, 운영 절차 및 사용자 작업 흐름 | Validator, View | 확인 | Warning은 평가 실패와 구분되어야 한다. |
| 필수 필드 검증 기준이 확정되었는가 | mission_config, capture_log, collision_log의 필수 필드가 고정되어야 한다. | 입력 파일 스키마 상세 설계, 요구사항-설계 상세 추적성 | Validator | 확인 | 필수 필드 규칙은 테스트와 직결된다. |
| 숫자 변환 기준이 확정되었는가 | 위치, 각도, 시간, 점수 관련 필드의 숫자 변환 규칙이 정해져 있어야 한다. | 용어 및 데이터 사전, 알고리즘 상세 설계 | Validator, ScoreCalculator | 확인 | 단위와 범위가 일치해야 계산이 가능하다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| 1:1 최적 매칭 기준이 확정되었는가 | Hungarian algorithm 기반의 전역 최적 기준이 고정되어야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | Matcher | 확인 | 매칭 전략 변경은 결과 전체에 영향을 준다. |
| 매칭 비용 항목이 확정되었는가 | 위치, 방향, 시간 조건을 어떤 순서와 가중치로 반영할지 정해져 있어야 한다. | 알고리즘 상세 설계, 용어 및 데이터 사전 | Matcher, ScoreCalculator | 확인 | 비용 항목이 바뀌면 판정과 감점이 달라진다. |
| missing 처리와 매칭의 경계가 확정되었는가 | 매칭 실패와 누락 판정을 분리하는 기준이 고정되어야 한다. | 알고리즘 상세 설계, 오류 처리 매트릭스 | Matcher, Evaluator | 확인 | 매칭 실패는 결과 집계에 직접 영향을 준다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| 평가 순서가 확정되었는가 | 로드, 검증, 매칭, 판정, 집계 순서가 고정되어야 한다. | 알고리즘 상세 설계, 운영 절차 및 사용자 작업 흐름 | Evaluator, AppController | 확인 | 순서가 바뀌면 상태 전이도 바뀐다. |
| position_ok, direction_ok, time_ok 판정 기준이 확정되었는가 | 각 bool 판정 조건이 명확해야 한다. | 용어 및 데이터 사전, 알고리즘 상세 설계 | Evaluator, TargetResult | 확인 | 판정 조건은 결과의 핵심 기준이다. |
| 평균 오차 집계 기준이 확정되었는가 | missing target 제외 원칙이 일치해야 한다. | 용어 및 데이터 사전, 요구사항-설계 상세 추적성 | Evaluator, EvalResult | 확인 | 집계 기준이 다르면 리포트가 흔들린다. |
| 상태 전이와 예외 처리 기준이 확정되었는가 | Evaluating, Completed, Error 전환이 정리되어야 한다. | 평가 상태 전이도, 오류 처리 매트릭스 | Evaluator, AppController | 확인 | 예외 처리 기준은 UI와 함께 봐야 한다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| 감점 항목이 확정되었는가 | position, yaw, pitch, timeout, missing, collision 감점이 정리되어야 한다. | 알고리즘 상세 설계, 용어 및 데이터 사전 | ScoreCalculator, ScoreDetail | 확인 | 감점 구조가 바뀌면 final_score도 바뀐다. |
| 감점 합산 방식이 확정되었는가 | total_deduction와 final_score 계산식이 고정되어야 한다. | 결과 집계 설계, 설계 결정 기록 | ScoreCalculator, EvalResult | 확인 | 합산 방식은 리포트와 시험의 기준이다. |
| 점수 범위가 확정되었는가 | final_score 범위와 하한/상한 처리 방식이 정해져 있어야 한다. | 용어 및 데이터 사전, 결과 집계 설계 | ScoreCalculator | 확인 | 범위 규칙이 없으면 결과 해석이 어렵다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| 4종 리포트 파일명이 확정되었는가 | eval_result.json, eval_summary.json, eval_result.csv, eval_detail.csv가 고정되어야 한다. | 파일명 및 리포트 용어, 요구사항-설계 상세 추적성 | ReportExporter | 확인 | 파일명은 외부 검토와 저장 시험의 기준이다. |
| JSON/CSV 출력 구조가 확정되었는가 | 상세/요약 구분이 명확해야 한다. | 리포트 저장 상세 설계, 용어 및 데이터 사전 | ReportExporter, EvalResult | 확인 | 출력 구조가 바뀌면 테스트 기대값도 바뀐다. |
| 저장 실패 처리 기준이 확정되었는가 | 결과 유지, Error 전환, 재시도 정책이 정리되어야 한다. | 오류 처리 매트릭스, 운영 절차 및 사용자 작업 흐름 | ReportExporter, AppController, View | 확인 | 저장 실패는 계산 결과와 분리해야 한다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| 탭 구성이 확정되었는가 | 파일 선택, 임무 설정 확인, 평가 실행, 결과 요약, 상세 결과, 시각화, 리포트 저장 탭이 고정되어야 한다. | UI 탭별 상세 설계, 운영 절차 및 사용자 작업 흐름 | View, AppController | 확인 | 탭 이름은 문서 전체에서 동일해야 한다. |
| 상태 메시지 기준이 확정되었는가 | Idle, FilesSelected, Evaluating, Completed, Error 메시지 흐름이 정리되어야 한다. | 평가 상태 전이도, 오류 처리 매트릭스 | View, AppController | 확인 | 상태 메시지는 사용자의 판단 기준이다. |
| 미리보기와 시각화의 경계가 확정되었는가 | 이미지 미리보기와 오차/감점 그래프의 역할이 구분되어야 한다. | UI 탭별 상세 설계, 용어 및 데이터 사전 | View | 확인 | 기능 중복은 혼동을 만든다. |
...
| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 관련 모듈 | 완료 여부 | 비고 |
|---|---|---|---|---|---|
| STD 시험 항목이 확정되었는가 | 입력, 출력, 상태, 성능 시험 항목이 정리되어 있어야 한다. | 요구사항-설계 상세 추적성, 테스트 설계 및 추적성 매트릭스 | STD, tests | 확인 | 시험 항목이 없으면 구현 완료 판정이 불가능하다. |
| 성공/실패 판정 기준이 확정되었는가 | 검증 기준과 기대 결과가 동일해야 한다. | 용어 및 데이터 사전, 요구사항-설계 상세 추적성 | STD, tests | 확인 | 테스트는 문서 기준을 그대로 반영해야 한다. |
| 성능 테스트 조건이 확정되었는가 | 100MB 이하, 30초 이내 조건이 고정되어야 한다. | 성능 및 확장성 설계, 요구사항-설계 상세 추적성 | STD, tests | 확인 | 성능 기준이 없으면 회귀 검증이 불가능하다. |
...
이 섹션은 SDD 내부 일관성, SRS-SDD 정합성, SDD-STD 정합성, 데이터 모델 적합성, 알고리즘 타당성, UI 설명 가능성, 오류 처리 일관성, 리포트 출력 일치성, 테스트 가능성, 유지보수성을 검토하기 위한 기준을 제공한다. 문서 검토는 형식 점검이 아니라 실제 구현과 시험의 기준이 되는 설계 품질 점검이다.

### 31.254 검토 대상 문서 범위

- SRS 드론 항공촬영 임무 평가 시스템 문서
- SDD 드론 항공촬영 임무 평가 시스템 문서
- STD 드론 항공촬영 임무 평가 시스템 문서
- 입력 파일 스키마 설명
- 데이터 모델 설명
- 알고리즘 상세 설계
- UI 탭별 상세 설계
...
검토 대상은 문서 본문과 표, 다이어그램, 용어 정의, 추적성 표, 완료 기준을 모두 포함한다. 구현 파일은 검토 대상에 포함하지 않으며, 문서 간 일치 여부만 확인한다.

### 31.255 SDD 내부 일관성 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| 동일한 필드명이 문서 전체에서 같은 의미로 사용되는가 | mission_name, target_id, position, direction, timestamp, image_path, final_score 등의 의미가 문서 전반에서 동일해야 한다. | 용어 및 데이터 사전, 데이터 모델 관계도, 요구사항-설계 상세 추적성 | 해당 필드를 정의한 모든 섹션을 동시에 수정한다. | 검토 전 |
| 좌표 단위가 meter로 일관되는가 | 위치 값은 meter 단위로만 설명해야 한다. | 용어 및 데이터 사전, 데이터 모델 관계도, 알고리즘 상세 설계 | 단위 표기와 계산식을 함께 수정한다. | 검토 전 |
| 각도 단위가 degree로 일관되는가 | yaw와 pitch는 degree로만 설명해야 한다. | 용어 및 데이터 사전, 알고리즘 상세 설계 | 각도 표기와 오차 계산 설명을 함께 수정한다. | 검토 전 |
| 시간 단위가 second로 일관되는가 | time_limit과 timestamp는 second 기준으로 일관되어야 한다. | 용어 및 데이터 사전, 입력 파일 스키마 상세 설계 | 시간 단위와 비교 조건을 함께 수정한다. | 검토 전 |
| success 판정 조건이 모든 섹션에서 동일한가 | 위치, 방향, 시간 조건의 결합 방식이 동일해야 한다. | 알고리즘 상세 설계, 용어 및 데이터 사전, 구현 준비 체크리스트 | 판정 조건과 관련 시험 항목을 함께 수정한다. | 검토 전 |
| missing target 처리 규칙이 모든 섹션에서 동일한가 | missing의 정의와 집계 기준이 동일해야 한다. | 알고리즘 상세 설계, 오류 처리 매트릭스, 용어 및 데이터 사전 | missing 정의와 평균 오차 설명을 함께 수정한다. | 검토 전 |
| collision 처리 규칙이 모든 섹션에서 동일한가 | collision=true의 의미와 감점 반영이 동일해야 한다. | 오류 처리 매트릭스, 용어 및 데이터 사전, 설계 결정 기록 | collision 정의와 감점 규칙을 함께 수정한다. | 검토 전 |
| deduction 필드명이 일관되는가 | position_deduction, yaw_deduction, pitch_deduction, total_deduction가 동일한 의미로 쓰여야 한다. | 용어 및 데이터 사전, 결과 집계 설계 | 감점 필드명과 계산 설명을 함께 수정한다. | 검토 전 |
| 출력 파일명이 문서 전체에서 동일한가 | eval_result.json, eval_summary.json, eval_result.csv, eval_detail.csv가 일치해야 한다. | 파일명 및 리포트 용어, 리포트 저장 상세 설계 | 파일명, 저장 절차, 테스트 항목을 함께 수정한다. | 검토 전 |

이 표는 문서 내부의 용어, 계산, 출력, 상태 정의가 서로 충돌하지 않는지 확인하기 위한 기준이다. 한 항목이라도 어긋나면 관련 섹션 전체를 함께 점검해야 한다.

### 31.256 SRS-SDD 일관성 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| SRS의 기능 요구사항이 SDD 설계 섹션에 대응되는가 | 기능 요구사항마다 대응하는 설계 섹션이 있어야 한다. | 요구사항-설계 상세 추적성, 알고리즘 상세 설계, UI 탭별 상세 설계 | 대응 섹션이 없으면 SDD를 보완한다. | 검토 전 |
| SRS의 입력 요구사항이 SDD 입력 스키마와 일치하는가 | mission_config.json, capture_log.csv, collision_log.csv, image_path가 일치해야 한다. | 입력 파일 스키마 상세 설계, 용어 및 데이터 사전 | 스키마와 용어를 SRS 기준으로 재정렬한다. | 검토 전 |
| SRS의 출력 요구사항이 SDD 리포트 설계와 일치하는가 | 출력 파일명과 필드 구성이 SRS와 같아야 한다. | 리포트 저장 상세 설계, 파일명 및 리포트 용어 | 출력 파일명과 필드 정의를 함께 수정한다. | 검토 전 |
| SRS의 UI 요구사항이 SDD UI 설계와 일치하는가 | 7개 탭과 상태 흐름이 SRS와 일치해야 한다. | UI 탭별 상세 설계, 운영 절차 및 사용자 작업 흐름 | UI 탭명과 화면 설명을 함께 수정한다. | 검토 전 |
| SRS의 비기능 요구사항이 SDD 성능/보안/안정성 설계와 연결되는가 | 100MB 이하, 30초 이내, UTF-8, 보안, 오류 처리 조건이 연결되어야 한다. | 성능 및 확장성 설계, 보안 및 안정성 설계, 오류 처리 매트릭스 | 비기능 요구사항과 연결된 설계 섹션을 보완한다. | 검토 전 |

이 표는 요구사항이 설계로 실제 번역되었는지 확인하기 위한 표이다. SRS에 있지만 SDD에 없는 내용은 누락으로 본다.

### 31.257 SDD-STD 일관성 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| SDD의 각도 계산 설계가 STD 테스트 항목과 연결되는가 | yaw wrap-around, pitch 절댓값 차이 기준이 STD에 반영되어야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | 테스트 항목과 계산식을 함께 수정한다. | 검토 전 |
| SDD의 점수 계산 설계가 STD 테스트 항목과 연결되는가 | 감점과 final_score 계산이 시험 항목으로 존재해야 한다. | 결과 집계 설계, 설계 결정 기록 | 점수 계산 설명과 시험 항목을 함께 수정한다. | 검토 전 |
| SDD의 매칭 설계가 STD 테스트 항목과 연결되는가 | Hungarian 매칭과 missing 판정이 시험 가능해야 한다. | 알고리즘 상세 설계, 요구사항-설계 상세 추적성 | 매칭 기준과 시험 데이터를 함께 수정한다. | 검토 전 |
| SDD의 입력 검증 설계가 STD 테스트 항목과 연결되는가 | 필수 필드, 헤더, 타입, 경로, 충돌 해석이 시험되어야 한다. | 오류 처리 매트릭스, 입력 파일 스키마 상세 설계 | 입력 검증 기준과 시험 항목을 함께 수정한다. | 검토 전 |
| SDD의 리포트 저장 설계가 STD 테스트 항목과 연결되는가 | 4종 리포트 저장과 파일명이 시험 가능해야 한다. | 리포트 저장 상세 설계, 파일명 및 리포트 용어 | 저장 형식과 시험 기대값을 함께 수정한다. | 검토 전 |

이 표는 설계가 시험으로 검증 가능한 형태인지 확인하기 위한 표이다. STD에 없는 설계는 실제 검증이 불가능하므로 보완이 필요하다.

### 31.258 데이터 모델 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| MissionConfig 필드가 입력 스키마와 일치하는가 | 목표, 허용 오차, 감점 정책 필드가 입력 파일과 대응되어야 한다. | 데이터 모델 관계도, 입력 파일 스키마 상세 설계 | 데이터 모델과 스키마를 함께 수정한다. | 검토 전 |
| CaptureRecord 필드가 capture_log.csv와 일치하는가 | timestamp, position, direction, image_path가 일치해야 한다. | 데이터 모델 관계도, 입력 파일 스키마 상세 설계 | 필드명과 파싱 설명을 함께 수정한다. | 검토 전 |
| CollisionRecord 필드가 collision_log.csv와 일치하는가 | collision 필드가 CSV 구조와 일치해야 한다. | 데이터 모델 관계도, 입력 파일 스키마 상세 설계 | 충돌 로그 구조와 모델을 함께 수정한다. | 검토 전 |
| TargetResult 필드가 eval_detail.csv와 일치하는가 | target별 판정, 오차, 감점 필드가 일치해야 한다. | 결과 집계 설계, 파일명 및 리포트 용어 | 상세 결과 필드와 리포트 설명을 함께 수정한다. | 검토 전 |
| EvalResult 필드가 eval_result.json과 일치하는가 | 집계 필드와 최종 점수가 JSON 구조와 일치해야 한다. | 결과 집계 설계, 파일명 및 리포트 용어 | 집계 필드와 JSON 구조를 함께 수정한다. | 검토 전 |
| None 허용 필드가 명확히 정의되어 있는가 | 허용 None 필드와 필수 필드가 구분되어야 한다. | 용어 및 데이터 사전, 데이터 모델 관계도 | None 허용 기준을 문서에 보강한다. | 검토 전 |
| bool 판정 필드가 명확히 정의되어 있는가 | position_ok, direction_ok, time_ok, missing, success, collision의 의미가 명확해야 한다. | 용어 및 데이터 사전, 알고리즘 상세 설계 | bool 판정 정의와 시험 항목을 함께 수정한다. | 검토 전 |

이 표는 데이터 모델이 입력, 계산, 출력과 끊김 없이 연결되는지 확인하기 위한 표이다. 모델이 흔들리면 모든 서비스와 UI가 연쇄적으로 영향을 받는다.

### 31.259 알고리즘 설계 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| 위치 오차 계산식이 명확한가 | 3D Euclidean distance 기준이 문서화되어 있어야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | 계산식과 예시를 함께 수정한다. | 검토 전 |
| yaw wrap-around 처리 기준이 명확한가 | -180도와 180도 경계 처리 기준이 명확해야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | 각도 보정 규칙을 함께 수정한다. | 검토 전 |
| pitch 오차 계산 기준이 명확한가 | pitch는 절댓값 차이로 계산한다는 기준이 있어야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | pitch 계산 설명을 함께 수정한다. | 검토 전 |
| 비용 함수가 명확한가 | 매칭 비용이 어떤 요소로 구성되는지 문서화되어야 한다. | 알고리즘 상세 설계 | 비용 함수와 관련 시험을 함께 수정한다. | 검토 전 |
| 허용 오차와 시간 조건이 매칭 후 판정된다는 원칙이 유지되는가 | 후보 사전 제외가 아니라 매칭 후 판정임이 유지되어야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | 판정 순서와 테스트 항목을 함께 수정한다. | 검토 전 |
| 감점 계산식이 명확한가 | 각 감점 항목과 합산 방식이 정리되어 있어야 한다. | 결과 집계 설계, 설계 결정 기록 | 감점 계산식과 리포트 설명을 함께 수정한다. | 검토 전 |
| final_score 하한 0점 처리가 명확한가 | final_score가 0점 미만으로 내려가지 않는 규칙이 있어야 한다. | 결과 집계 설계, 용어 및 데이터 사전 | 하한 처리와 시험 항목을 함께 수정한다. | 검토 전 |

이 표는 알고리즘이 결과를 설명할 수 있는 수준으로 문서화되었는지 확인하기 위한 표이다. 계산식이 अस्पष्ट하면 구현은 동일해도 결과 검증이 불가능해진다.

### 31.260 UI 설계 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| 7개 탭의 목적이 명확한가 | 파일 선택, 임무 설정 확인, 평가 실행, 결과 요약, 상세 결과, 시각화, 리포트 저장 탭의 목적이 구분되어야 한다. | UI 탭별 상세 설계, 운영 절차 및 사용자 작업 흐름 | 탭 목적 설명과 화면 요소를 함께 수정한다. | 검토 전 |
| 각 탭의 Controller 호출이 명확한가 | 탭별로 어떤 Controller 메서드가 호출되는지 드러나야 한다. | UI 탭별 상세 설계, 운영 절차 및 사용자 작업 흐름 | Controller 연동 설명을 보완한다. | 검토 전 |
| 평가 중 버튼 비활성화 기준이 명확한가 | Evaluating 상태에서 실행 버튼이 비활성화되어야 한다. | 평가 상태 전이도, 오류 처리 매트릭스 | 버튼 상태와 상태 전이 설명을 함께 수정한다. | 검토 전 |
| Error 상태에서 사용자 조치가 명확한가 | 입력 수정, 경로 수정, 재저장, 재평가 조치가 구분되어야 한다. | 운영 절차 및 사용자 작업 흐름, 오류 처리 매트릭스 | Error 상태 조치와 UI 문구를 함께 수정한다. | 검토 전 |
| 결과 요약과 상세 결과 표시 항목이 명확한가 | 요약 수치와 target별 상세 항목이 구분되어야 한다. | UI 탭별 상세 설계, 용어 및 데이터 사전 | 요약/상세 표시 항목을 함께 수정한다. | 검토 전 |
| 이미지 미리보기 실패 처리 기준이 명확한가 | 미리보기 실패는 Warning으로 다루어야 한다. | UI 탭별 상세 설계, 오류 처리 매트릭스 | 미리보기 오류 메시지와 시험 항목을 함께 수정한다. | 검토 전 |

이 표는 UI가 사용 흐름과 상태 전이를 올바르게 반영하는지 확인하기 위한 표이다. 화면 목적이 불분명하면 사용자 절차와 시험이 모두 흔들린다.

### 31.261 오류 처리 설계 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| Fatal 오류 처리 기준이 명확한가 | 입력 구조 붕괴, 파싱 실패, 진행 불가 오류가 Fatal로 구분되어야 한다. | 오류 처리 매트릭스, 운영 절차 및 사용자 작업 흐름 | Fatal 정의와 상태 전이를 함께 수정한다. | 검토 전 |
| Recoverable 오류 처리 기준이 명확한가 | 일부 행 오류와 경고 후 진행 기준이 정리되어야 한다. | 오류 처리 매트릭스, 입력 파일 스키마 상세 설계 | Recoverable 기준과 입력 검증 설명을 함께 수정한다. | 검토 전 |
| Warning 처리 기준이 명확한가 | image_path 문제나 미리보기 실패가 Warning으로 분류되어야 한다. | 오류 처리 매트릭스, 운영 절차 및 사용자 작업 흐름 | Warning 기준과 UI 메시지를 함께 수정한다. | 검토 전 |
| Info 처리 기준이 명확한가 | 진행 안내와 상태 알림이 Info로 구분되어야 한다. | 오류 처리 매트릭스, UI 탭별 상세 설계 | Info 용도와 상태 메시지를 함께 수정한다. | 검토 전 |
| Error 상태 전이 기준이 명확한가 | 치명적 실패 시 Error 상태로 전환되어야 한다. | 평가 상태 전이도, 오류 처리 매트릭스 | Error 전이와 재시도 규칙을 함께 수정한다. | 검토 전 |
| 오류 목록 관리 기준이 명확한가 | 오류 누적, 표시, 정렬, 요약 기준이 있어야 한다. | 오류 처리 매트릭스, 운영 절차 및 사용자 작업 흐름 | 오류 목록 구조와 UI 표시를 함께 수정한다. | 검토 전 |
| UI 오류 메시지 기준이 명확한가 | 사용자용 메시지와 내부 디버그 메시지가 분리되어야 한다. | 오류 처리 매트릭스, UI 탭별 상세 설계 | 메시지 정책과 표시 위치를 함께 수정한다. | 검토 전 |

이 표는 오류를 분류하는 기준과 UI 반영 기준이 같은지 확인하기 위한 표이다. 오류 분류가 달라지면 검증과 사용자 조치도 달라진다.

### 31.262 리포트 출력 설계 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| 출력 파일명이 문서 전반에서 동일한가 | eval_result.json, eval_summary.json, eval_result.csv, eval_detail.csv가 일치해야 한다. | 파일명 및 리포트 용어, 리포트 저장 상세 설계 | 파일명과 저장 설명을 함께 수정한다. | 검토 전 |
| 출력 구조가 요약과 상세로 구분되는가 | JSON과 CSV에서 요약/상세 구분이 유지되어야 한다. | 리포트 저장 상세 설계, 결과 집계 설계 | 출력 구조와 시험 기대값을 함께 수정한다. | 검토 전 |
| 저장 경로와 저장 실패 처리 기준이 명확한가 | 저장 실패 시 결과 유지와 재시도 원칙이 있어야 한다. | 리포트 저장 상세 설계, 운영 절차 및 사용자 작업 흐름 | 저장 실패 규칙과 UI 문구를 함께 수정한다. | 검토 전 |

이 표는 리포트가 문서와 외부 파일 간의 일관된 인터페이스가 되는지 확인하기 위한 표이다. 저장 형식은 검토와 재현성의 기준이 된다.

### 31.263 테스트 가능성 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| 각 요구사항이 STD 시험 항목으로 변환되는가 | 기능, 비기능, 오류, 성능 요구사항이 시험 항목과 연결되어야 한다. | 요구사항-설계 상세 추적성, 테스트 설계 및 추적성 매트릭스 | STD와 추적성 표를 함께 보완한다. | 검토 전 |
| 입력 검증이 테스트 데이터로 재현되는가 | 필수 필드, 타입, 범위, 경로, 인코딩 검증이 시험 가능해야 한다. | 입력 파일 스키마 상세 설계, 오류 처리 매트릭스 | 테스트 데이터와 검증 기준을 함께 수정한다. | 검토 전 |
| 매칭과 점수 계산이 기대값으로 비교되는가 | expected score와 오차 값이 명시되어야 한다. | 알고리즘 상세 설계, 결과 집계 설계 | 기대값과 계산식을 함께 수정한다. | 검토 전 |
| 리포트 파일이 결과 비교로 검증되는가 | JSON/CSV 내용이 문서 필드와 대응되어야 한다. | 리포트 저장 상세 설계, 파일명 및 리포트 용어 | 리포트 구조와 비교 항목을 함께 수정한다. | 검토 전 |
| 성능 시험이 측정 가능한가 | 100MB 이하, 30초 이내 조건이 측정 가능해야 한다. | 성능 및 확장성 설계, 요구사항-설계 상세 추적성 | 성능 기준과 측정 환경을 함께 수정한다. | 검토 전 |

이 표는 설계가 실제 시험으로 이어질 수 있는지 확인하기 위한 표이다. 시험으로 측정할 수 없는 설계는 검증 가능성이 낮다.

### 31.264 유지보수성 점검표

| 점검 항목 | 확인 기준 | 관련 SDD 섹션 | 불일치 시 조치 | 완료 여부 |
|---|---|---|---|---|
| 모듈 책임이 분리되어 있는가 | Model, Service, Controller, View 책임이 겹치지 않아야 한다. | 모듈 의존성 다이어그램, 기능별 모듈 설명 | 책임 중복 부분을 재배치한다. | 검토 전 |
| 용어 사전이 참조 기준으로 쓰이는가 | 동일 용어가 여러 섹션에서 같은 의미로 사용되어야 한다. | 용어 및 데이터 사전, 요구사항-설계 상세 추적성 | 용어 정의를 중앙화해 수정한다. | 검토 전 |
| 변경 영향 범위가 추적되는가 | 미확정 사항 및 변경 관리 설계와 연결되어야 한다. | 미확정 사항 및 변경 관리 설계, 설계 결정 기록 | 영향 범위 표를 보완한다. | 검토 전 |
| 신규 요구사항이 추가되기 쉬운 구조인가 | 향후 확장 항목이 별도 섹션으로 분리되어 있어야 한다. | 성능 및 확장성 설계, 미확정 사항 및 변경 관리 설계 | 확장 후보와 변경 기준을 함께 수정한다. | 검토 전 |

이 표는 유지보수자가 문서를 기준으로 변경할 때 혼란을 줄이기 위한 표이다. 책임 분리가 흐려지면 이후 수정 비용이 커진다.

### 31.265 최종 승인 기준

- 문서가 5000줄 수준의 상세 설계 문서가 되었는가.
- 주요 설계 산출물이 모두 포함되었는가.
- 구현자가 SDD를 기준으로 모듈 구현을 시작할 수 있는가.
- 테스트 작성자가 STD와 SDD를 기준으로 테스트를 작성할 수 있는가.
- 미확정 사항이 명확히 분리되어 있는가.
- 문서 간 모순이 발견되면 수정 후 승인한다.

최종 승인은 단순히 검토자가 만족하는 상태가 아니라, 설계와 시험과 구현이 같은 언어를 사용한다는 확인이다. 하나라도 어긋나면 승인보다 수정이 우선이다.

### 31.266 검토 후 수정 원칙

- 불일치가 발견되면 해당 섹션만 고치지 말고 연결된 표와 용어를 함께 수정한다.
- 필드명, 단위, 판정 조건, 파일명은 독립적으로 바꾸지 않고 연관 문맥을 함께 수정한다.
- SRS, SDD, STD 간 모순이 있으면 상위 요구사항을 먼저 확인한다.
- 테스트 가능성이 낮은 설계는 구현보다 먼저 문서에서 정리한다.
...
이 섹션은 본 SDD가 어떤 방향으로 확장되었는지 기록하고, 최종 검토 시점에 구현 전 전환 여부를 판단하기 위한 기준을 남기기 위한 것이다. 변경 이력은 과거 기록을 장황하게 나열하는 용도가 아니라, 현재 문서가 구현 기준으로 사용 가능한지 확인하기 위한 근거가 된다.

### 31.282 변경 이력 기록 기준

- 변경 이력은 실제 이번 SDD 확장 작업 중심으로 기록한다.
- 각 항목은 새로 추가된 설계 산출물과 문서 색인을 기준으로 요약한다.
- 변경 목적은 기능 추가가 아니라 설계 기준의 명시와 추적성 확보로 적는다.
- 구현 영향과 테스트 영향은 연결된 섹션을 기준으로 정리한다.
- 검토 상태는 최종 검토 전 기준으로 관리한다.

### 31.283 주요 확장 이력 표

| 변경 항목 | 변경 목적 | 관련 SDD 섹션 | 구현 영향 | 테스트 영향 | 검토 상태 |
|---|---|---|---|---|---|
| Data Flow Diagram 추가 | 전체 입력, 처리, 출력 흐름을 구조화한다. | 전체 시스템 Data Flow Diagram | 입력 처리와 결과 흐름의 기준이 생긴다. | 데이터 흐름 시험 기준이 생긴다. | 검토 전 |
| Sequence Diagram 추가 | 평가 실행 순서를 명시한다. | 평가 실행 Sequence Diagram | Controller와 Service 호출 순서가 명확해진다. | 실행 순서 시험 기준이 생긴다. | 검토 전 |
| 상태 전이도 추가 | 상태 변화와 전이 조건을 정리한다. | 평가 상태 전이도 | UI와 Controller 상태 관리가 명확해진다. | 상태 전이 시험 기준이 생긴다. | 검토 전 |
| 모듈 의존성 다이어그램 추가 | 모듈 책임과 연결 관계를 정리한다. | 모듈 의존성 다이어그램 | 모듈 경계가 분명해진다. | 의존성 시험 기준이 생긴다. | 검토 전 |
| 데이터 모델 관계도 추가 | 데이터 구조 간 연결을 정리한다. | 데이터 모델 관계도 | 데이터 모델 구현 기준이 생긴다. | 모델 관계 시험 기준이 생긴다. | 검토 전 |
| 입력 파일 스키마 상세 설계 추가 | 입력 필드와 타입을 고정한다. | 입력 파일 스키마 상세 설계 | FileLoader와 Validator 기준이 생긴다. | 입력 파싱 시험 기준이 생긴다. | 검토 전 |
| 오류 처리 매트릭스 추가 | 오류 등급과 처리 원칙을 정리한다. | 오류 처리 매트릭스 | 예외 처리와 상태 전이가 명확해진다. | 오류 처리 시험 기준이 생긴다. | 검토 전 |
| Public Method 명세 추가 | 서비스 인터페이스 책임을 정리한다. | 모듈별 Public Method 명세 | 모듈 경계와 호출 책임이 명확해진다. | 인터페이스 시험 기준이 생긴다. | 검토 전 |
| 알고리즘 상세 설계 추가 | 매칭, 오차, 감점 계산을 명시한다. | 알고리즘 상세 설계 | Evaluator와 ScoreCalculator 기준이 생긴다. | 알고리즘 시험 기준이 생긴다. | 검토 전 |
| 리포트 저장 상세 설계 추가 | 출력 형식과 저장 규칙을 정리한다. | 리포트 저장 상세 설계 | ReportExporter 기준이 생긴다. | 저장 시험 기준이 생긴다. | 검토 전 |
| UI 탭별 상세 설계 추가 | 화면 구성과 동작을 정리한다. | UI 탭별 상세 설계 | View와 AppController 기준이 생긴다. | UI 동작 시험 기준이 생긴다. | 검토 전 |
| 테스트 설계 및 추적성 매트릭스 추가 | 요구사항과 시험을 연결한다. | 테스트 설계 및 추적성 매트릭스 | 구현과 시험의 연결 기준이 생긴다. | 추적성 시험 기준이 생긴다. | 검토 전 |
| 실행 시나리오 상세 설계 추가 | 정상, 오류, 재평가 흐름을 정리한다. | 실행 시나리오 상세 설계 | 운영 흐름 이해 기준이 생긴다. | 시나리오 시험 기준이 생긴다. | 검토 전 |
| 성능 및 확장성 설계 추가 | 입력 크기와 처리 시간 기준을 정리한다. | 성능 및 확장성 설계 | 성능 구현 기준이 생긴다. | 성능 시험 기준이 생긴다. | 검토 전 |
| 보안 및 안정성 설계 추가 | 입력 안전성과 안정성 기준을 정리한다. | 보안 및 안정성 설계 | 안전한 입력 처리 기준이 생긴다. | 보안 및 안정성 시험 기준이 생긴다. | 검토 전 |
| 설계 결정 기록 추가 | 주요 설계 선택의 근거를 남긴다. | 설계 결정 기록 | 구현 선택 근거가 명확해진다. | 설계 검토 기준이 생긴다. | 검토 전 |
| 미확정 사항 및 변경 관리 설계 추가 | 변경 후보와 확정 기준을 분리한다. | 미확정 사항 및 변경 관리 설계 | 변경 통제 기준이 생긴다. | 변경 관리 시험 기준이 생긴다. | 검토 전 |
| 용어 및 데이터 사전 추가 | 용어와 필드 의미를 고정한다. | 용어 및 데이터 사전 | 필드명과 상태 해석 기준이 생긴다. | 용어 일치 시험 기준이 생긴다. | 검토 전 |
| 요구사항-설계 상세 추적성 추가 | 요구사항과 설계를 연결한다. | 요구사항-설계 상세 추적성 | 요구사항 누락 점검 기준이 생긴다. | 추적성 시험 기준이 생긴다. | 검토 전 |
| 운영 절차 및 사용자 작업 흐름 추가 | 사용자 흐름과 상태 전이를 정리한다. | 운영 절차 및 사용자 작업 흐름 | UI와 Controller 흐름이 명확해진다. | 사용자 흐름 시험 기준이 생긴다. | 검토 전 |
| 구현 준비 체크리스트 및 완료 기준 추가 | 구현 착수와 완료 조건을 정의한다. | 구현 준비 체크리스트 및 완료 기준 | 구현 시작 전 점검 기준이 생긴다. | 구현 준비 시험 기준이 생긴다. | 검토 전 |
| 문서 검토 기준 및 품질 점검표 추가 | 문서 정합성과 품질 기준을 정리한다. | 문서 검토 기준 및 품질 점검표 | 최종 검토 기준이 명확해진다. | 문서 검토 시험 기준이 생긴다. | 검토 전 |
| 부록 및 문서 색인 추가 | 설계 산출물과 색인을 정리한다. | 부록: 설계 산출물 목록 및 문서 색인 | 탐색성과 참조성이 향상된다. | 색인 기반 확인이 가능해진다. | 검토 전 |

이 표는 이번 확장 작업에서 무엇이 추가되었는지를 한눈에 보여준다. 실제 구현 변경이 아니라 설계 문서 확장 기록이므로, 각 항목의 목적은 구현 요구가 아니라 문서 기준의 명시이다.

### 31.284 최종 검토 메모

- 본 SDD는 구현 코드보다 먼저 확정되어야 하는 설계 기준이다.
- 구현은 SDD에 정의된 데이터 모델, 입력 스키마, 알고리즘, 리포트 형식, UI 흐름을 따른다.
- SRS 또는 STD와 모순되는 항목이 발견되면 구현 전에 문서를 먼저 수정한다.
- 미확정 사항은 구현자가 임의로 결정하지 않는다.
- 문서 검토 기준 및 품질 점검표에 따라 최종 검토 후 구현으로 전환한다.

이 메모는 설계와 구현 사이의 우선순위를 다시 한 번 고정한다. 문서가 기준이며, 구현은 그 기준을 따르는 후속 단계이다.

### 31.285 구현 전 최종 확인 사항

- 현재 문서 줄 수가 5000줄 수준에 도달했는가.
- 입력 파일 구조가 확정되었는가.
- 데이터 모델이 확정되었는가.
- 매칭 및 점수 계산 알고리즘이 확정되었는가.
- 오류 처리 기준이 확정되었는가.
- 리포트 출력 형식이 확정되었는가.
- UI 탭 구성과 상태 전이가 확정되었는가.
- 테스트 추적성이 확보되었는가.
- 미확정 사항이 별도 관리되고 있는가.

이 확인 사항은 구현 착수 전 마지막 점검 목록이다. 하나라도 미확인이라면 구현보다 문서 보완이 우선이다.

### 31.286 검토 보류 사항

- flight_log.csv 반영 시점
- 리포트 자동 저장 여부
- 진행률 계산 방식
- 저장 실패 후 상태 유지 정책
- 대용량 매칭 최적화 전략
...
이 항목들은 현재 문서에서 변경 관리 대상으로 남겨 둔다. 구현 판단으로 즉시 확정하지 않고, 미확정 사항 관리 절차를 통해 다룬다.

### 31.287 다음 단계 전환 기준

- SDD 줄 수 목표 달성
- SRS/SDD/STD 간 필드명 충돌 없음
- 입력/출력 스키마 확정
- 구현 착수 금지 조건 해소
- 우선 구현 범위 확정
- 테스트 우선순위 확정

이 기준이 충족되면 문서 검토에서 구현 착수 단계로 전환할 수 있다. 기준이 충족되지 않으면 추가 구현보다 문서 정합성 보완이 먼저다.

## 최종 승인 체크

### 31.288 최종 승인 체크의 목적

이 섹션은 구현 시작 직전의 마지막 검토 기준을 고정하기 위한 것이다. 승인 여부를 문서 전체의 정합성과 검증 가능성으로 판단하고, 구현자가 임의로 기준을 바꾸지 않도록 체크 항목을 명확히 남긴다.
...
승인 대상 범위는 구현 코드가 아니라 문서 자체이다. 따라서 코드 상태보다 문서의 정합성과 검증 기준을 우선 확인한다.

### 31.290 승인 전 확인 표

| 확인 항목 | 확인 기준 | 관련 섹션 | 상태 | 비고 |
|---|---|---|---|---|
| 입력 스키마가 확정되었는가 | mission_config.json, capture_log.csv, collision_log.csv, image_path 기준이 고정되어 있어야 한다. | 입력 파일 스키마 상세 설계, 용어 및 데이터 사전 | 검토 전 | 입력 구조가 흔들리면 전체 설계가 흔들린다. |
| 데이터 모델이 확정되었는가 | MissionConfig, CaptureRecord, CollisionRecord, TargetResult, ScoreDetail, EvalResult가 고정되어 있어야 한다. | 데이터 모델 관계도, 용어 및 데이터 사전 | 검토 전 | 모델이 먼저 고정되어야 한다. |
| 매칭 알고리즘이 확정되었는가 | 1:1 최적 매칭과 비용 기준이 정리되어 있어야 한다. | 알고리즘 상세 설계, 설계 결정 기록 | 검토 전 | 매칭 기준이 결과 집계에 직결된다. |
| 점수 계산 방식이 확정되었는가 | 감점 항목과 final_score 계산식이 문서화되어 있어야 한다. | 결과 집계 설계, 알고리즘 상세 설계 | 검토 전 | 점수 구조가 명확해야 한다. |
| 오류 처리 기준이 확정되었는가 | Fatal, Recoverable, Warning, Error 상태 전이가 정리되어 있어야 한다. | 오류 처리 매트릭스, 평가 상태 전이도 | 검토 전 | 오류 분류와 상태 전이가 연결되어야 한다. |
| 리포트 출력 형식이 확정되었는가 | 4종 리포트 파일명과 구조가 고정되어 있어야 한다. | 리포트 저장 상세 설계, 파일명 및 리포트 용어 | 검토 전 | 파일명은 외부 검토 기준이다. |
| UI 탭 구성이 확정되었는가 | 7개 탭과 역할이 일관되어야 한다. | UI 탭별 상세 설계, 운영 절차 및 사용자 작업 흐름 | 검토 전 | 탭 구성은 사용자 흐름의 기준이다. |
| 테스트 추적성이 확보되었는가 | 요구사항과 STD, tests의 연결이 보여야 한다. | 요구사항-설계 상세 추적성, 테스트 설계 및 추적성 매트릭스 | 검토 전 | 추적성 없이는 승인할 수 없다. |
| 미확정 사항이 분리되어 있는가 | 변경 관리 대상이 별도 섹션으로 분리되어 있어야 한다. | 미확정 사항 및 변경 관리 설계 | 검토 전 | 미확정 사항은 구현 기준에서 분리해야 한다. |
| 구현 착수 금지 조건이 정리되어 있는가 | 착수 금지 조건이 명시되어 있어야 한다. | 구현 준비 체크리스트 및 완료 기준 | 검토 전 | 금지 조건이 없으면 승인 보류가 어렵다. |

이 표는 승인 직전에 빠르게 확인할 핵심 항목만 모은 것이다. 하나라도 미확인이라면 승인보다 보완이 우선이다.

### 31.291 구현 전 확인 표

| 확인 항목 | 확인 기준 | 관련 섹션 | 상태 | 비고 |
|---|---|---|---|---|
| FileLoader 구현 기준 확인 | 입력 파일 로드와 경로 처리 기준이 정리되어 있어야 한다. | FileLoader 구현 준비 체크리스트 | 검토 전 | 입력 경계 모듈부터 확인한다. |
| Validator 구현 기준 확인 | Fatal, Recoverable, Warning 구분이 정리되어 있어야 한다. | Validator 구현 준비 체크리스트 | 검토 전 | 오류 분류가 선행되어야 한다. |
| Matcher 구현 기준 확인 | 1:1 최적 매칭 기준이 정리되어 있어야 한다. | Matcher 구현 준비 체크리스트 | 검토 전 | 매칭 기준이 결과에 직결된다. |
| Evaluator 구현 기준 확인 | 평가 순서와 판정 기준이 정리되어 있어야 한다. | Evaluator 구현 준비 체크리스트 | 검토 전 | 실행 흐름의 중심이다. |
| ScoreCalculator 구현 기준 확인 | 감점과 final_score 계산 기준이 정리되어 있어야 한다. | ScoreCalculator 구현 준비 체크리스트 | 검토 전 | 점수 기준이 흔들리면 안 된다. |
| ReportExporter 구현 기준 확인 | 4종 리포트 저장 기준이 정리되어 있어야 한다. | ReportExporter 구현 준비 체크리스트 | 검토 전 | 외부 출력의 기준이다. |
| AppController 구현 기준 확인 | 상태 전이와 흐름 조정 기준이 정리되어 있어야 한다. | 운영 절차 및 사용자 작업 흐름, 평가 상태 전이도 | 검토 전 | UI와 Service를 연결한다. |
| View 구현 기준 확인 | 7개 탭과 상태 메시지 기준이 정리되어 있어야 한다. | UI 탭별 상세 설계, 운영 절차 및 사용자 작업 흐름 | 검토 전 | 사용자 가시 영역을 우선 본다. |

이 표는 구현 전에 각 모듈의 설계 기준이 충분한지 확인하는 표이다. 구현 기준이 빈약하면 승인하지 않는다.

### 31.292 테스트 전 확인 표

| 확인 항목 | 확인 기준 | 관련 섹션 | 상태 | 비고 |
|---|---|---|---|---|
| angle_utils 테스트 기준 확인 | yaw wrap-around와 pitch 차이 기준이 정리되어 있어야 한다. | 알고리즘 상세 설계, 용어 및 데이터 사전 | 검토 전 | 각도 계산의 기준 시험이다. |
| score_calculator 테스트 기준 확인 | 감점과 final_score 계산 기준이 정리되어 있어야 한다. | 결과 집계 설계, 알고리즘 상세 설계 | 검토 전 | 점수 계산의 기준 시험이다. |
| matcher 테스트 기준 확인 | 1:1 최적 매칭과 missing 판정 기준이 정리되어 있어야 한다. | 알고리즘 상세 설계, 요구사항-설계 상세 추적성 | 검토 전 | 매칭 결과의 기준 시험이다. |
| evaluator 테스트 기준 확인 | 평가 흐름과 집계 기준이 정리되어 있어야 한다. | 알고리즘 상세 설계, 테스트 설계 및 추적성 매트릭스 | 검토 전 | 통합 판정의 기준 시험이다. |
| file_loader 테스트 기준 확인 | 입력 파일 로드와 인코딩 기준이 정리되어 있어야 한다. | 입력 파일 스키마 상세 설계, 성능 및 확장성 설계 | 검토 전 | 입력 로드의 기준 시험이다. |
| validator 테스트 기준 확인 | 필수 필드와 오류 등급 기준이 정리되어 있어야 한다. | 오류 처리 매트릭스, 입력 파일 스키마 상세 설계 | 검토 전 | 검증 기준의 기준 시험이다. |
| report_exporter 테스트 기준 확인 | 4종 리포트 저장 기준이 정리되어 있어야 한다. | 리포트 저장 상세 설계, 파일명 및 리포트 용어 | 검토 전 | 출력 파일의 기준 시험이다. |
| 통합 테스트 기준 확인 | 입력부터 저장까지의 전 과정 기준이 정리되어 있어야 한다. | 실행 시나리오 상세 설계, 테스트 설계 및 추적성 매트릭스 | 검토 전 | 전체 흐름의 기준 시험이다. |

이 표는 테스트가 실제 설계 문서를 기준으로 작성될 수 있는지 확인하기 위한 표이다. 기준이 없으면 테스트도 승인하지 않는다.

### 31.293 문서 간 정합성 확인 표

| 확인 항목 | 확인 기준 | 관련 섹션 | 상태 | 비고 |
|---|---|---|---|---|
| SRS와 입력 요구사항 일치 여부 | 입력 파일명, 필드명, 구조가 SRS와 일치해야 한다. | 입력 파일 스키마 상세 설계, 요구사항-설계 상세 추적성 | 검토 전 | 입력 정의부터 확인한다. |
| SRS와 출력 요구사항 일치 여부 | 출력 파일명과 내용 구조가 SRS와 일치해야 한다. | 리포트 저장 상세 설계, 파일명 및 리포트 용어 | 검토 전 | 출력 정의를 함께 확인한다. |
| SRS와 UI 요구사항 일치 여부 | 탭 구성과 사용자 흐름이 SRS와 일치해야 한다. | UI 탭별 상세 설계, 운영 절차 및 사용자 작업 흐름 | 검토 전 | 화면 기준을 확인한다. |
| STD와 테스트 항목 일치 여부 | STD의 시험 항목이 tests와 연결되어야 한다. | 테스트 설계 및 추적성 매트릭스, 요구사항-설계 상세 추적성 | 검토 전 | 시험 항목 연결을 확인한다. |
| SDD 내부 필드명 일치 여부 | 동일 필드명이 모든 섹션에서 같은 의미여야 한다. | 용어 및 데이터 사전, 데이터 모델 관계도 | 검토 전 | 필드명 충돌을 확인한다. |
| SDD 내부 단위 일치 여부 | meter, degree, second 단위가 일관되어야 한다. | 용어 및 데이터 사전, 알고리즘 상세 설계 | 검토 전 | 단위 충돌을 확인한다. |

이 표는 문서끼리 같은 언어를 사용하는지 확인하기 위한 표이다. 정합성은 승인 전 필수 조건이다.

### 31.294 최종 승인 조건

- SDD가 5000줄 수준에 도달한다.
- SRS/SDD/STD 간 명백한 모순이 없다.
- 구현자가 SDD 기준으로 모듈 구현을 시작할 수 있다.
- 테스트 작성자가 SDD와 STD 기준으로 테스트를 작성할 수 있다.
- 미확정 사항이 구현 판단으로 임의 처리되지 않도록 분리되어 있다.

최종 승인 조건은 문서가 구현과 시험의 기준으로 충분한지 판단하는 마지막 기준이다. 조건이 충족되지 않으면 승인보다 수정이 우선이다.

### 31.295 승인 보류 조건

- 입력 스키마가 불명확하다.
- success, missing, timeout 판정 기준이 불명확하다.
- 리포트 출력 필드명이 불명확하다.
- 오류 처리 정책이 문서 간 충돌한다.
- 테스트 기준이 설계와 연결되지 않는다.
